...
from .message_utils import abort_result, extract_text, message_group_id, message_id, message_session_id, normalize_text
...
class SleepCoreMixin:
    ...
    def _is_control_command(self, message: dict[str, Any]) -> bool:
        ...
        if text in {"/sleep_status", "/sleep_wake"}:
            self._allow_control_reply()
            return True
        return False

    def _is_control_reply(self, message: dict[str, Any]) -> bool:
        """判断出站消息是否为本插件控制命令的回复"""
